...
// It takes an image name, the port and the arguments that we need.
func StartContainer(image string, port string, args ...string) (*Container, error) {
	// The "-P" means give us a random port and "-d" detach from the process "run in the backgorund".
	arg := []string{"run", "-P", "-d"}
	arg = append(arg, args...)
	arg = append(arg, image)

	// We execute the docker command with the arguments
